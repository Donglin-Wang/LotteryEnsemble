...
class ClientGenesis(Client):
    ...
    def client_update(self, global_model, global_init_model, round):
        ...
        for epoch in range(self.args.client_epoch):
            train_score = train(round, self.client_id, epoch, self.model,
                                self.train_loader,
                                lr=self.args.lr,
        ...
        self.losses[round] = np.array(losses)
        self.accuracies[round] = np.array(accuracies)
        self.prune_rates[round] = cur_prune_rate
...
class ServerGenesis(Server):
    ...
    def server_update(self):
        ...
        for round in range(0, self.comm_rounds):
            print('-------------------------------------', flush=True)
            print(f'Communication Round #{round}', flush=True)
            print('-------------------------------------', flush=True)
            for c in [self.clients[i] for i in np.random.choice(self.num_clients,
                                                                max(int(self.frac * self.num_clients), 1),
                                                                replace=False)]:
                c.client_update(self.global_models, self.global_init_model, round)
            ...
            self.accuracies[round] = eval_score['Accuracy'][-1]
            # gather client accuracies
            for k, m in enumerate(self.clients):
                self.client_accuracies[k][round] = m.evaluate()

            # prune global model if appropriate
            num_pruned, num_params = get_prune_summary(self.global_models)
            cur_prune_rate = num_pruned / num_params
            if self.client_accuracies[:, round].mean() > self.args.acc_thresh and cur_prune_rate < self.args.prune_percent:
